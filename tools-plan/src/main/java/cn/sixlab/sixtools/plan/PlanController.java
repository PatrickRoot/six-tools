...
public class PlanController extends BaseController implements Initializable {
    ...
    private ToolsPlan thePlan = null;
    ...
    private Callback<TableColumn, TableCell> tableColorCallback() {
        return p -> new TableCell<ToolsPlan, String>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                ...
                if (!isEmpty()) {
                    ...
                    ToolsPlan colorPlan = (ToolsPlan) tableRow.getItem();
                    if (null != colorPlan) {
    ...
    private EventHandler<MouseEvent> tableClick(TableView table) {
        return e -> {
            if (e.getButton().equals(MouseButton.PRIMARY) && e.getClickCount() == 2) {
                ...
                ToolsPlan clickedPlan = (ToolsPlan) table.getSelectionModel().getSelectedItem();
    ...
    private void showContextMenu(TableView table, MouseEvent e) {
        ToolsPlan selectedPlan = (ToolsPlan) table.getSelectionModel().getSelectedItem();
    ...
    private void confirmStartPlan(ToolsPlan toolsPlan) {
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("确认");
        alert.setHeaderText("确定重启任务：" + toolsPlan.getPlanName());
        alert.setContentText(toolsPlan.getPlanContent());

        Optional<ButtonType> result = alert.showAndWait();

        if (result.get() == ButtonType.OK) {
            toolsPlan.setPlanStatus(C.PLAN_STATUS_ING);
            dao.update(toolsPlan);
            searchAllTableData();
        }
    }

    private void confirmStopPlan(ToolsPlan toolsPlan) {
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("确认");
        alert.setHeaderText("确定终止任务：" + toolsPlan.getPlanName());
        alert.setContentText(toolsPlan.getPlanContent());

        Optional<ButtonType> result = alert.showAndWait();

        if (result.get() == ButtonType.OK) {
            toolsPlan.setPlanStatus(C.PLAN_STATUS_STOP);
            dao.update(toolsPlan);
            searchAllTableData();
        }
    }

    private void confirmFinishPlan(ToolsPlan toolsPlan) {
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("确认");
        alert.setHeaderText("确定完成任务：" + toolsPlan.getPlanName());
        alert.setContentText(toolsPlan.getPlanContent());

        Optional<ButtonType> result = alert.showAndWait();

        if (result.get() == ButtonType.OK) {
            toolsPlan.setPlanStatus(C.PLAN_STATUS_ED);
            dao.update(toolsPlan);
            searchAllTableData();
        }
    }
    ...
    public void searchYearSer() {
        ...
        yearData.clear();
        yearData.addAll(dao.query(ToolsPlan.class, cnd));
    }
    ...
    public void searchSeasonSer() {
        ...
        seasonData.clear();
        seasonData.addAll(dao.query(ToolsPlan.class, cnd));
    }
    ...
    public void searchMonthSer() {
        ...
        monthData.clear();
        monthData.addAll(dao.query(ToolsPlan.class, cnd));
    }
    ...
    public void searchWeekSer() {
        ...
        weekData.clear();
        weekData.addAll(dao.query(ToolsPlan.class, cnd));
    }
